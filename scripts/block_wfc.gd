...
func initBlock(block_coord: Vector2):
	...
	for x in range(block_dim.x):
		for y in range(block_dim.y):
			...
			#if tile_coord not in tile_grid: 
				#tile_grid[tile_coord] = ""
			#if abs(fmod(block_coord.x, 2.0)) == abs(fmod(block_coord.y, 2.0)):
				#tile_grid[tile_coord] = tile_grid[tile_coord] + "a"
			#else:
				#tile_grid[tile_coord] = tile_grid[tile_coord] + "z"
...
func collapseBlock(block_coord: Vector2):
	...
	var collapsed_block = wfc.doWaveFunctionCollapse()
